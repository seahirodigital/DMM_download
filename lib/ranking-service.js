...
function extractDmmLitevideoCidCandidatesFromUrl(value) {
  if (!value) {
    return [];
  }
  ...
  try {
    ...
    const values = [
      ...
      /\/product_id=([^/?#]+)/i.exec(url.pathname)?.[1],
      /\/content_id=([^/?#]+)/i.exec(url.pathname)?.[1]
    ];
    return values.flatMap(getDmmLitevideoCidVariants);
  } catch {
    return [];
  }
}
...
function getDmmAffiliateLitevideoCidCandidates(item) {
  const values = [
    item?.content_id,
    item?.product_id,
    item?.cid,
    item?.itemCode,
    ...extractDmmLitevideoCidCandidatesFromUrl(item?.URL),
    ...extractDmmLitevideoCidCandidatesFromUrl(item?.affiliateURL)
  ];
  return [...new Set(values.flatMap(getDmmLitevideoCidVariants).filter(Boolean))];
}

function buildDmmLitevideoPlaybackUrl(config, cid) {
  const qualitySize = LITEVIDEO_QUALITY_SIZES[0];
  const affiId = config?.affiliate?.searchAffiliateId || config?.affiliate?.affiliateId || '';
  const affiPath = affiId ? `/affi_id=${encodeURIComponent(affiId)}` : '';
  return `https://www.dmm.co.jp/litevideo/-/part/=/cid=${encodeURIComponent(cid)}/size=${qualitySize}${affiPath}/`;
}

function getDmmAffiliateItemPlaybackUrl(config, item, target, source) {
  if (isFanzaItemListTarget(target) || !isDmmAffiliateSearchSource(source)) {
    return '';
  }

  const cid = getDmmAffiliateLitevideoCidCandidates(item)[0];
  return cid ? buildDmmLitevideoPlaybackUrl(config, cid) : '';
}
...
function getAffiliateItemPlaybackUrl(config, item, target, source) {
  ...
  return getDmmAffiliateItemPlaybackUrl(config, item, target, source);
}
...
function mapAffiliateItems(config, apiItems, options = {}) {
  ...
  return apiItems.map((item, index) => {
    ...
    const resolvedSource = source || (isFanzaItemListTarget(target) ? 'fanza-actress-search' : 'affiliate-actress-search');
    ...
    return {
      ...
      fetchedAt,
      fetchedDateLabel: dateLabel,
      playbackUrl: getAffiliateItemPlaybackUrl(config, item, target, resolvedSource),
      productCode,
