...
function isDmmDvdItemListTarget(target = {}) {
  return (
    String(target.site || '').toLowerCase() === 'dmm.com' &&
    String(target.service || '').toLowerCase() === 'mono' &&
    String(target.floor || '').toLowerCase() === 'dvd'
  );
}
...
function extractCidFromAffiliateUrl(value) {
  try {
    const url = new URL(value);
    const cidParam =
      ...
      '';
    return cidParam ? decodeURIComponent(cidParam) : '';
  } catch {
    return '';
  }
}

function normalizeLitevideoCid(value) {
  return String(value || '')
    .normalize('NFKC')
    .trim()
    .replace(/\.[a-z0-9]+$/i, '')
...
function buildLitevideoPlaybackUrlFromCid(config, cid) {
  const normalizedCid = normalizeLitevideoCid(cid);
  if (!normalizedCid) {
    return '';
  }

  const url = new URL('https://www.dmm.co.jp/litevideo/-/part/=/');
  const parts = [`cid=${encodeURIComponent(normalizedCid)}`, `size=${LITEVIDEO_QUALITY_SIZES[0]}`];
  if (config.affiliate.searchAffiliateId) {
    parts.push(`affi_id=${encodeURIComponent(config.affiliate.searchAffiliateId)}`);
  }
  url.pathname = `/litevideo/-/part/=/${parts.join('/')}/`;
  return url.toString();
}

function getDmmKeywordItemLitevideoUrl(config, item, target, source) {
  if (source !== 'affiliate-keyword-search' || !isDmmDvdItemListTarget(target)) {
    return '';
  }
  ...
  const candidates = [
    ...
    item?.cid,
    item?.itemCode,
    extractCidFromAffiliateUrl(item?.URL),
    extractCidFromAffiliateUrl(item?.affiliateURL)
  ];

  for (const candidate of candidates) {
    const playbackUrl = buildLitevideoPlaybackUrlFromCid(config, candidate);
    if (playbackUrl) {
      return playbackUrl;
    }
  }
...
function getAffiliateItemPlaybackUrl(config, item, target, source) {
  ...
  return preferHighQualityLitevideoUrl(playbackUrl) || getDmmKeywordItemLitevideoUrl(config, item, target, source);
}
