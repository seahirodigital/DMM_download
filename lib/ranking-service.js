...
function getActressItemListTargets(config) {
  return [...getItemListTargets(config)].sort((left, right) => {
...
function mapAffiliateItems(config, apiItems, options = {}) {
  ...
  return apiItems.map((item, index) => {
    ...
    return {
      ...
      searchUrl: buildSearchUrl(config, title, actress),
      source: isFanzaItemListTarget(target) ? 'fanza-actress-search' : 'affiliate-actress-search',
      sourcePageUrl,
...
async function fetchAffiliateActressSearch(config, options = {}) {
  ...
  const itemListTargets = getActressItemListTargets(config);
  ...
  async function collectItemsForTarget(target, mode) {
    ...
    while (page < maxPages) {
      ...
      const pageItems = result.items.filter((item) => {
        ...
        if (!getAffiliateItemPlaybackUrl(item)) {
          return false;
        }
