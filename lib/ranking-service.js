...
function getAffiliateItemLitevideoUrl(config, item, target) {
  if (!isDmmDvdItemListTarget(target)) {
    return '';
  }
...
function getAffiliateItemPlaybackUrl(config, item, target) {
  const sample = item?.sampleMovieURL || {};
  const playbackUrl = getBestSampleMovieUrl(sample);
  return preferHighQualityLitevideoUrl(playbackUrl) || getAffiliateItemLitevideoUrl(config, item, target);
}
...
function mapAffiliateItems(config, apiItems, options = {}) {
  ...
  return apiItems.map((item, index) => {
    ...
    return {
      ...
      fetchedAt,
      fetchedDateLabel: dateLabel,
      playbackUrl: getAffiliateItemPlaybackUrl(config, item, target),
      productCode,
