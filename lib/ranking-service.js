...
async function fetchActressSearch(config, options = {}) {
  let tvSearchError = null;

  try {
    const tvSearch = await fetchTvActressSearch(config, options);
    if (tvSearch.items?.length || !hasAffiliateApiCredentials(config)) {
      return tvSearch;
    }
  } catch (error) {
    tvSearchError = error;
  ...
  const affiliateSearch = await fetchAffiliateActressSearch(config, options);
  return tvSearchError
    ? {
        ...affiliateSearch,
        fallbackReason: tvSearchError.message || String(tvSearchError)
      }
    : affiliateSearch;
}
