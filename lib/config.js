...
const DEFAULT_CONFIG = {
  ...
  affiliate: {
    ...
    itemListHits: 100,
    makerSearchEndpoint: 'https://api.dmm.com/affiliate/v3/MakerSearch',
    makerSearchFloorId: '43',
    makerSearchHits: 100,
    makerSearchMaxPages: 50
  },
  downloads: {
    defaultLimit: 5,
    concurrency: 1,
...
async function loadConfig() {
  ...
  const makerSearchHits = Number(process.env.DMM_MAKER_SEARCH_HITS || merged.affiliate.makerSearchHits);
  const makerSearchMaxPages = Number(process.env.DMM_MAKER_SEARCH_MAX_PAGES || merged.affiliate.makerSearchMaxPages);
  ...
  return {
    ...
    affiliate: {
      ...
      makerSearchEndpoint: process.env.DMM_MAKER_SEARCH_ENDPOINT || merged.affiliate.makerSearchEndpoint,
      makerSearchFloorId: process.env.DMM_MAKER_SEARCH_FLOOR_ID || merged.affiliate.makerSearchFloorId || '43',
      makerSearchHits: Number.isFinite(makerSearchHits) ? Math.max(1, Math.min(100, makerSearchHits)) : 100,
      makerSearchMaxPages: Number.isFinite(makerSearchMaxPages) ? Math.max(1, Math.min(100, makerSearchMaxPages)) : 50
    },
    paths: {
      ...merged.paths,
