...
query FetchSearchVideosForPreview($device: Device!, $keyword: String, $sort: VideoSearchSortKey, $first: Int) {
  searchVideos(device: $device, keyword: $keyword, sort: $sort, first: $first) {
    edges {
      node {
        id
        titleName
...
function getDmmLitevideoCidVariants(value) {
  ...
  const variants = [];
  const add = (candidate) => {
    const normalizedCandidate = normalizeDmmLitevideoCid(candidate);
    if (normalizedCandidate && !variants.includes(normalizedCandidate)) {
      variants.push(normalizedCandidate);
    }
  };
  const addBase = (base) => {
    const compactedNumber = /^(.*[a-z])0+(\d+[a-z]?)$/i.exec(base);
    if (compactedNumber) {
      add(`${compactedNumber[1]}${compactedNumber[2].replace(/^0+(?=\d)/, '')}`);
    }
    add(base);
  };

  const baseCandidates = [
    getDmmDvdMakerPrefixCid(normalized),
    normalized,
    normalized.includes('_') ? normalized.replace(/_/g, '') : ''
  ].filter(Boolean);

  for (const baseCandidate of baseCandidates) {
    addBase(baseCandidate);
    if (baseCandidate.endsWith('dl') && baseCandidate.length > 2) {
      addBase(baseCandidate.slice(0, -2));
    }
  }
...
function litevideoUrlWithCid(value, cid) {
  try {
    const url = new URL(value);
    if (/\/cid=[^/]+/i.test(url.pathname)) {
      url.pathname = url.pathname.replace(/\/cid=[^/]+/i, `/cid=${encodeURIComponent(cid)}`);
      return url.toString();
    }
...
function getLitevideoCidUrlVariants(value) {
  try {
    const url = new URL(value);
    const cid = /\/cid=([^/]+)/i.exec(url.pathname)?.[1] || url.searchParams.get('cid') || '';
    const cidVariants = getDmmLitevideoCidVariants(decodeURIComponent(cid));
    const urls = cidVariants.map((candidate) => litevideoUrlWithCid(url.toString(), candidate)).filter(Boolean);
...
function getLitevideoQualityUrls(value) {
  const urls = [];
  for (const urlVariant of getLitevideoCidUrlVariants(value)) {
    urls.push(...LITEVIDEO_QUALITY_SIZES.map((qualitySize) => litevideoUrlWithQuality(urlVariant, qualitySize)));
    urls.push(urlVariant);
  }
...
function normalizeDmmTvLookupTitle(value) {
  return String(value || '')
    .normalize('NFKC')
    .trim()
    .toLowerCase()
...
function getDmmTvLookupTitleKeywords(item) {
  const values = [item?.title, item?.rawTitle]
    .map((value) => String(value || '').normalize('NFKC').trim())
    .filter((value) => value.length >= 2);
  return [...new Set(values)];
}

function getDmmTvLookupTitleKeys(item) {
  return getDmmTvLookupTitleKeywords(item)
    .map(normalizeDmmTvLookupTitle)
    .filter((value) => value.length >= 2);
}
...
function scoreDmmTvLookupTitleMatch(node, titleKeys) {
  const nodeTitle = normalizeDmmTvLookupTitle([node?.titleName, node?.seasonName].filter(Boolean).join(' '));
  if (!nodeTitle || !titleKeys.length) {
    return 0;
  }
  ...
  for (const titleKey of titleKeys) {
    ...
    if (nodeTitle === titleKey) {
      score = Math.max(score, 100);
    } else if (nodeTitle.includes(titleKey)) {
      score = Math.max(score, 90);
    } else if (titleKey.includes(nodeTitle) && nodeTitle.length >= 12) {
...
function pickDmmTvLookupMatch(edges, lookupCodes, titleKeys = []) {
  ...
  const titleMatches = nodes
    .map((node) => ({
      node,
      score: scoreDmmTvLookupTitleMatch(node, titleKeys)
    }))
    .filter((entry) => entry.score > 0)
    .sort((left, right) => right.score - left.score);
  if (titleMatches.length) {
    const best = titleMatches[0];
    const next = titleMatches[1];
    if (best.score >= 90 || !next || best.score > next.score) {
      return best.node;
    }
  }
...
async function fetchGraphqlSampleSourceByContentId(item, config) {
  ...
  const titleKeys = getDmmTvLookupTitleKeys(item);
  const lookupKeywords = [...new Set([...lookupCodes, ...getDmmTvLookupTitleKeywords(item)])].filter(Boolean);
  if (!lookupKeywords.length) {
    return null;
  }

  for (const keyword of lookupKeywords.slice(0, 8)) {
    const edges = await fetchDmmTvLookupEdges(keyword, config).catch(() => []);
    const matched = pickDmmTvLookupMatch(edges, lookupCodes, titleKeys);
    if (!matched?.id) {
      continue;
    }
...
async function resolvePlayableSource(item, config) {
  ...
  if (item?.playbackUrl && isLitevideoPageUrl(item.playbackUrl)) {
    const litevideoReferer = shouldPreferDmmTvContentLookup(item) ? '' : detailUrl;
    const litevideoSource = await resolveLitevideoPageSource(item.playbackUrl, litevideoReferer, config).catch(() => null);
    if (litevideoSource?.url) {
      return litevideoSource;
    }
  }
  ...
  if (!triedContentIdGraphqlSource && (shouldPreferDmmTvContentLookup(item) || (!item?.source && !item?.playbackUrl))) {
  ...
  if (shouldPreferDmmTvContentLookup(item)) {
    throw new Error(
      'DMM検索結果の再生用URLを確認できませんでした。検索結果はDMM Affiliate ItemListを維持し、再生はsampleMovieURLまたはlitevideo候補のみを使います。'
    );
  }
