...
class StateStore {
  constructor(config) {
    this.config = config;
    this.hosted = config.appMode === 'hosted';
    ...
    this.favoritesFile = path.join(this.runtimeDir, 'favorites.json');
  ...
  async init() {
    ...
    this.favorites = this.normalizeFavorites(await readJson(this.favoritesFile, {}));
    this.downloadHistory = await readJson(this.downloadHistoryFile, []);
  }

  normalizeFavorites(value) {
    ...
    return Object.fromEntries(
      Object.entries(value).filter(([key, item]) => {
        return typeof key === 'string' && key.trim() && item && typeof item === 'object' && !Array.isArray(item);
      })
    );
  }
  ...
  async setFavorites(favorites) {
    this.favorites = this.normalizeFavorites(favorites);
    if (!this.hosted) {
      await writeJson(this.favoritesFile, this.favorites);
    }
